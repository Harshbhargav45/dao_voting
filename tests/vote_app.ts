...
import * as idl from "../target/idl/vote_app.json";
...
describe("Testing the voting app", () => {
  ...
  const adminWallet = anchor.web3.Keypair.fromSecretKey(
    Uint8Array.from(JSON.parse(require("fs").readFileSync(require("os").homedir() + "/.config/solana/id.json", "utf-8")))
  );
  const provider = new anchor.AnchorProvider(connection, new NodeWallet(adminWallet), { commitment: "confirmed" });
  ...
  describe("6.Pick Winner", () => {
    ...
    it("6.2 Should pick winner after deadline passes", async () => {
      ...
      expect(winnerData.winningVotes.toNumber()).to.equal(1);
    });
  })
